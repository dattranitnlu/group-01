...
const { TestDetail } = require('../models/db')
...
router.get('/', (req, res) => {
    TestDetail.findAll().then(type => {
        res.json(Result(type))
    });
});

router.get('/:id', (req, res) => { //d+ là những con số, bắt buộc
    TestDetail.findByPk(req.params.id).then(type => {
        if (type != null) {
            res.json(Result(type));
        } else {
...
router.post('/', (req, res) => { //create 
    //validate data here
    TestDetail.create(req.body).then(type => {
        res.json(Result(type));
    }).catch(err => {
...
router.put('/:testid', (req, res) => { //updating
    //validate data here
    TestDetail.findByPk(req.params.testid).then(type => {
        if (type != null) {
            type.update({
